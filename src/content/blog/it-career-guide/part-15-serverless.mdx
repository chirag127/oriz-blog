...
```mermaid
graph TD
    subgraph Container Cold Start (AWS Lambda)
    bootVM["1. Spin up VM container"] --> bootRuntime["2. Load runtime (Node/Python VM)"]
    bootRuntime --> runCode["3. Execute code (100ms - 5000ms delay)"]
    end
    subgraph V8 Isolate Startup (Cloudflare Workers)
    loadIsolate["1. Mount V8 Isolate (Shared RAM runtime)"] --> runIsolate["2. Execute JS context (<5ms delay)"]
    end
```
