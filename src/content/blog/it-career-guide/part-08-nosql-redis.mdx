...
```mermaid
graph TD
    subgraph Embedding 1-to-N
    parentDoc1["User Document (Parent)<br>{ _id: 1, name: 'Chirag' }"]
    parentDoc1 -->|"Embeds"| addressArray["Addresses: [ { street: 'Main St', city: 'Delhi' } ]"]
    end
    subgraph Referencing 1-to-Unbounded
    parentDoc2["Post Document (Parent)<br>{ _id: 101, title: 'Kafka' }"]
    commentDoc1["Comment 1<br>{ _id: 1, postId: 101, text: 'Great post!' }"] -->|"References"| parentDoc2
    commentDoc2["Comment 2<br>{ _id: 2, postId: 101, text: 'Awesome' }"] -->|"References"| parentDoc2
    end
```
